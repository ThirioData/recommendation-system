...
spice_feat = pd.read_excel('test10.xls')
...
new_user_feat = pd.read_csv('new_user_feat111.csv')
...
while(new_user):
    ...
    recom = [0,0,0,0,0]
    pastorder = pd.DataFrame([[user_guid,last5ayreco,last5ayreco,recom,recom,recom,recom,[0,0,0,0,0,0,0]]],columns=['user_guid','last_6_days_bought','last_6_days_recommend','recommends','bought','not_recommended'])
...
while next_day :
    ...
    if entry==1:
        ...
        indexer = indexer[0]
        new_meal_size = main_recommend.main_recommendation(new_user_feat,user_feat,user_id,spice_feat)
        ...
        order_req = input('Do u want to order something?for yes type 1 else 0')
        ...
        if spice_feat.loc[order,'Place in Menu.1'] == 'Subji':
            spice_feat = spice_feat[spice_feat['Place in Menu.1'] == 'Subji']
            recommend = main_recommend.real_time_recommend(user_id,new_user_feat,spice_feat,user_feat)     
            new_user_feat.at[indexer, 'recommend_subji'] = recommend

        elif spice_feat.loc[order,'Place in Menu.1'] == 'Rice'  and (spice_feat['Place in Menu.1'] == 'Roti'):
            spice_feat = spice_feat[(spice_feat['Place in Menu.1'] == 'Rice') & (spice_feat['Place in Menu.1'] == 'Roti')]
            recommend = main_recommend.real_time_recommend(user_id,new_user_feat,spice_feat,user_feat)     
            new_user_feat.at[indexer, 'recommend_rice_roti'] = recommend
                            
        elif spice_feat['Place in Menu.1'] == 'Dal':    
            spice_feat = spice_feat[spice_feat['Place in Menu.1'] == 'Dal']
            recommend = main_recommend.real_time_recommend(user_id,new_user_feat,spice_feat,user_feat)     
            new_user_feat.at[indexer, 'recommend_dal'] = recommend
